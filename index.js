...
function showTableSet(body, set) {
  ...
  switch (natureLower) {
    case "lonely":
      {
        natureColors.atk = getTypeColor("grass");
        natureColors.def = getTypeColor("fire");
      }
      break;
    case "brave":
      {
        natureColors.atk = getTypeColor("grass");
        natureColors.spe = getTypeColor("fire");
      }
      break;
    case "adamant":
      {
        natureColors.atk = getTypeColor("grass");
        natureColors.spa = getTypeColor("fire");
      }
      break;
    case "naughty":
      {
        natureColors.atk = getTypeColor("grass");
        natureColors.spd = getTypeColor("fire");
      }
      break;
    case "bold":
      {
        natureColors.def = getTypeColor("grass");
        natureColors.atk = getTypeColor("fire");
      }
      break;
    case "relaxed":
      {
    ...
    case "timid":
      {
        natureColors.spe = getTypeColor("grass");
        natureColors.atk = getTypeColor("fire");
      }
      break;
    case "hasty":
    ...
    case "modest":
      {
        natureColors.spa = getTypeColor("grass");
        natureColors.atk = getTypeColor("fire");
      }
      break;
    case "mild":
    ...
    case "calm":
      {
        natureColors.spd = getTypeColor("grass");
        natureColors.atk = getTypeColor("fire");
      }
      break;
    case "gentle":
  ...
  for (move of set.moves) {
    ...
    switch (moveLower) {
      ...
      case "ivy cudgel":
        {
          // If species has second type
          if ("type2" in species) {
            // Same as secondary type (fire, water, rock)
            moveColor = getTypeColor(species.type2);
          } // Only one type
          else {
            // Same as primary type (grass)
            moveColor = getTypeColor(species.type1);
          }
        }
        break;
      // Tera Blast
...
function showPageTeam(format, folder, team) {
  ...
  // Export to clipboard event listener
  document
    .getElementById("coverage-export")
    .addEventListener("click", async (event) => {
      // Export string which will be copied to the clipboard
      const content = parseJson(data.sets);
...
function getColSprite(sets) {
  ...
  for (let set of sets) {
    ...
    if (it < 6) {
      ...
      if (set.other.hasOwnProperty("shiny") && set.other.shiny == "Yes") {
    ...
    if (m_includes_a == m_includes_b) {
      ...
      if (z_includes_a == z_includes_b) {
        ...
        if (g_includes_a == g_includes_b) {
          ...
          // A before B, return -1
          if (a_species > b_species) return -1;
          // B before A, return 1
          if (b_species > a_species) return 1;
          // Same, return 0
          return 0;
        }

        // Gmax > Non-Gmax
        if (g_includes_a) return -1;
        if (g_includes_b) return 1;
      }

      // Z > Non-Z
      if (z_includes_a) return -1;
      if (z_includes_b) return 1;
    }

    // Mega > Non-Mega
    if (m_includes_a) return -1;
    if (m_includes_b) return 1;
  }

  // Restricted > Non-Restricted
  if (r_includes_a) return -1;
  if (r_includes_b) return 1;
}
  ...
  copy.addEventListener("click", async (event) => {
    ...
    if (navigator.clipboard) {
      ...
      try {
        ...
        // Successful copy alert
        window.alert(
          "Team '" + data.name + "' copied to clipboard successfully."
        );
      } catch (err) {
        // Report the failure to the error console
        console.error(
